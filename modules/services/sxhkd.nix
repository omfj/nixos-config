    ...
    services = {
      sxhkd = {
        ...
        keybindings = {
          ...
          # bspwm move/resize
          "super + alt + {h, j, k, l}" = "bspc node -z {left -20 0, bottom 0 20, top 0 -20, right 20 0}";         # Make current node larger
          "super + alt + shift + {h, j, k, l}" = "bspc node -z {right -20 0, top 0 20, bottom 0 -20, left 20 0}"; # Made current node smaller
          "super + {Left, Down, Up, Right}" = "bspc node -v {-20 0, 0 20, 0 -20, 20 0}";                          # Move current node (floating)

          # Lock screen
          "super + x" = "betterlockscreen --lock blur"; # Lock screen
          ...
          "XF86AudioMicMute" = "amixer set Capture toggle -q"; # Toggle mute mic audio
